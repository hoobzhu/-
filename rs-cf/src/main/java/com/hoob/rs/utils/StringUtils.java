...
public final class StringUtils {
	...
	/**
	 * 将URL地址的路径和文件名部分Encode编码
	 * 
	 * @param srcURL
	 */
	public static String encodeUrl(String srcURL) {
		try {
			String decodeResut1 = decodeUrl(srcURL);// 第一遍解码
			if (StringUtils.isEmpty(decodeResut1) || !decodeResut1.equals(srcURL)) {// 如果第一遍解码结果为空，或者和第二遍解码结果不一样，那么表示已编码或为空，直接返回
				return srcURL;
			}
			String[] a1 = srcURL.split("://");
			if (a1.length < 2) {
				return srcURL;
			}
			...
			StringBuilder sb = new StringBuilder();
			sb.append(a1[0]);
			sb.append("://").append(a2[0]).append("/");
			for (int i = 1; i < a2.length; i++) {
				String temp = URLEncoder.encode(a2[i], "utf-8").replace("+", "%20");
				sb.append(temp);
	...
	public static String decodeUrl(String srcURL) {
		try {
			...
			sb.append("://").append(a2[0]).append("/");
			for (int i = 1; i < a2.length; i++) {
				String temp = URLDecoder.decode(a2[i], "utf-8");
				sb.append(temp);
