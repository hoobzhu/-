...
.controller('schedulerJobListControllers', ['$scope','$http','$routeParams','$location','$log','$window','sharedProperties','loginService','$translate','$rootScope','$cookieStore','commonMethod',function($scope, $http, $routeParams, $location, $log,$window, sharedProperties, loginService,$translate,$rootScope,$cookieStore,commonMethod) {
    ...
    //	1:cms分发  2：cdn 分发  3 :bms分发 4:epg  5  TVGW&TCGS 6：c2注入   7：c1注入  8：c3注入   9：其他  10:DRM  11:SeeLib  12:EPGTEMPLATE
    	 $scope.Gorup=[{'key':1,"name":'CMS'},
    	               {'key':2,"name":'CDN'},
    	               {'key':3,"name":'BMS'},
    	               {'key':4,"name":'EPG'},
    	               {'key':5,"name":'TVGW&TCGS'},
    	               {'key':10,"name":'DRM'},
    	               {'key':11,"name":'SeeLib'},
    	               {'key':6,"name":'C2'},
    	               {'key':7,"name":'C1'},
    	               {'key':8,"name":'C3'},
    	               {'key':12,"name":"EPGTEMPLATE"},
    	               {'key':9,"name":'other'}
    	               ];
    ...
    $scope.getPageData = function() {
        ...
        $scope.url = sharedProperties.getServerUrl()+"/rest/v1/sys/schdljobmng/list?jobgroup="+$scope.jobgroup
        // headers:{'X-Auth-Token':loginService.getToken()}
    	$http.get($scope.url).success(function (largeLoad) {
            $scope.items = largeLoad.schedulerJobList;
    ...
    $scope.selectModifyFlag = function() {
    	...
    	if(count==1){
    		// 判断item的状态 $scope.singleCheckItem.totalCdnStatus==1||
    ...
    $scope.toEnable=function(id,event){
         ...
         //	commonMethod.confirmDialog($scope.message).then(function(value) {
         		$http["post"](sharedProperties.getServerUrl()+"/rest/v1/sys/schdljobmng/enable/"+id)
    ...
    var	ids=[];
    	angular.forEach($scope.items, function(item) {
     	...
     	$http["post"](sharedProperties.getServerUrl()+"/rest/v1/sys/schdljobmng/bathenable?status="+status+"&ids="+ids)
            .success(function(data,status){
            	if(data.resultCode==0){
            		$scope.checkAll=false;
            		 $scope.getPageData();
...
.controller('schedulerJobEditControllers',['$scope','$routeParams','$log', '$http','sortKey','$location','$timeout','sharedProperties','fileUpload','$rootScope','$q','$interval','commonMethod',function($scope, $routeParams,$log, $http,sortKey, $location,$timeout,sharedProperties,fileUpload,$rootScope,$q,$interval,commonMethod) {
    ...
    $scope.init = function() {
		...
		$("#test").hide();
		$http.get(sharedProperties.getServerUrl()+"/rest/v1/sys/schdljobmng/detail/" + $scope.id).success(function (largeLoad) {
        	$scope.item = largeLoad.schedulerJobVO;
...
//			$scope.sValue = $scope.lrgValue + '/' + $scope.rrgValue;
    ...
    $scope.saveItem = function() {
    	...
    	var data = {"cronExpression": $scope.newExprsn,"id":$scope.item.id,"description":$scope.item.description,"status":$scope.item.status,"jobGroup":$scope.item.jobGroup};
    	$http.put(sharedProperties.getServerUrl()+'/rest/v1/sys/schdljobmng/edit',data).success(function(result) {
    	...
    	/* $.ajax({
	            url : sharedProperties.getServerUrl() + '/rest/v1/sys/schdljobmng/edit',
	            type : "post",
	            dataType : "json",
