		...
		[
				...
				function($scope, $http, $routeParams, $location, $log, $window, sharedProperties, loginService, $translate, $rootScope, $cookieStore, commonMethod,
					...
					$scope.saveItem = function(manFlag) {
						...
						if (1 == manFlag) {// 保存图片
							var url = sharedProperties.getServerUrl() + '/rest/v1/sys/picconfig/update';
							$scope.pics = {
								vod : $scope.vod,
							...
							$http["post"](sharedProperties.getServerUrl() + '/rest/v1/sys/contentid/add', $scope.datatmp, {
								headers : {
					...
					$scope.checkHead = function() {
						...
						if ($scope.data.head != null && $scope.data.head != "") {
							$http["get"](sharedProperties.getServerUrl() + "/rest/v1/sys/contentid/checkhead/" + $scope.data.head).success(function(data, status) {
					...
					$scope.init = function() {
						...
						if ($routeParams.manFlag != null && $routeParams.manFlag != undefined) {
							$scope.manFlag = $routeParams.manFlag;
						} else {
							$scope.manFlag = 2;
						}
						$scope.getPageData();
					...
					$scope.getPageData = function() {
						...
						if ($scope.manFlag == 0) {
							$http["get"](
									sharedProperties.getServerUrl() + '/rest/v1/sys/contentid/list?first=' + ($scope.pagingOptions.currentPage - 1) * $scope.pagingOptions.pageSize
											+ '&max=' + $scope.pagingOptions.pageSize + '&head=' + $scope.queryOptions.head + '&status=' + $scope.queryOptions.status).success(
									function(largeLoad) {
										...
										$scope.items = largeLoad.contentIdGeneratorList;

									});
						} else if ($scope.manFlag == 1) {
							$http.get(sharedProperties.getServerUrl() + '/rest/v1/sys/picconfig/list').success(function(meta) {
								// console.log(meta);
								var data = meta.vo;
								$scope.vod = data.vod;
								...
								$scope.specialtopic = data.specialtopic;
								$scope.ad = data.ad;
							});
							$scope.showPicGroup = $scope.vod;
							$scope.role = sessionStorage.getItem("role");
						} else if ($scope.manFlag == 2) {
						
							$scope.name = "";
							$http.get(sharedProperties.getServerUrl() + '/rest/v1/sys/config/list').success(function(largeLoad) {
								$scope.pagingOptions.total = largeLoad.list.length; // 存放list总记录数
								$scope.pagingOptions.maxPages = Math.ceil($scope.pagingOptions.total / $scope.pagingOptions.pageSize);
								$scope.items = largeLoad.list;
								// console.log(largeLoad);
							});
						} else if ($scope.manFlag == 3) {
							$scope.items = null;
							$scope.name = "";
							$http.get(sharedProperties.getServerUrl() + '/rest/v1/sys/config/constantlist').success(function(largeLoad) {
								$scope.pagingOptions.total = largeLoad.list.length; // 存放list总记录数
								$scope.pagingOptions.maxPages = Math.ceil($scope.pagingOptions.total / $scope.pagingOptions.pageSize);
								$scope.items = largeLoad.list;
								// console.log(largeLoad);
							});
						} else if ($scope.manFlag == 4) {
							$http.get(sharedProperties.getServerUrl() + '/rest/v1/sys/config/bykey?key=logo_pic_config').success(function(largeLoad) {
					...
					$scope.saveLogo = function() {
						var url = sharedProperties.getServerUrl() + '/rest/v1/sys/config/update';
						var temp = {
					...
					$scope.startStopConfig = function(cfg, event) {
						...
						var url = sharedProperties.getServerUrl() + "/rest/v1/sys/config/enable?id=" + cfg.id;
					...
					$scope.refresh = function() {
						var url = sharedProperties.getServerUrl() + '/rest/v1/sys/config/refresh';
						$http.put(url).success(function(data) {
							sysConfigService.clearCache();
							// alert("提交成功了");
							if (data.resultCode != 0) {
					...
					$scope.toContentEdit = function(config) {
						...
						window.location = sharedProperties.getServerUrl() + "/index.html#/sys/config/edit/" + config.id;
					};
					// 清理系統緩存
					$scope.clean = function() {
						var url = sharedProperties.getServerUrl() + '/rest/v1/sys/config/clean';
						$http.put(url).success(function(data) {
							sysConfigService.clearCache();
							// alert("提交成功了");
							if (data.resultCode != 0) {
		...
		[
				...
				function($rootScope, $scope, $modal, $log, $http, $routeParams, sharedProperties, loginService, $translate, $location, commonMethod) {
					...
					$scope.editRest = function() {
						$http.get(sharedProperties.getServerUrl() + '/rest/v1/sys/config/detail?id=' + $scope.id).success(function(result) {
					...
					// }
					//		
					// $scope.removeRow = function(value,index) {
					// //var cp = $scope.values[index];
