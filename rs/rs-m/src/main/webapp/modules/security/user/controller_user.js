...
.controller('userListCtrl', ['$rootScope','$scope', '$http', 'sharedProperties','loginService','$window','$routeParams','$q','$log','commonMethod',function($rootScope,$scope, $http, sharedProperties, loginService,$window,$routeParams,$q,$log,commonMethod) {
    $scope.enableUser = function(item){
    	...
    	///user/enableuser
    	var userid = item.id;
    	var url = sharedProperties.getServerUrl()+"/rest/v1/security/user/enable?id="+item.id;
    ...
    $scope.resetPassword = function(item){
    	...
    	///user/enableuser
    	var userid = item.id;
    	var url = sharedProperties.getServerUrl()+"/rest/v1/security/user/password/reset?id="+item.id;
		$http.post(url).success(function (data) {
			if(data.resultCode==0){
	...
	$scope.getPageData = function() {
		...
		var url = sharedProperties.getServerUrl()+"/rest/v1/security/user/list?id="+encodeURIComponent($scope.queryOptions.id)+"&name="+encodeURIComponent(name)+"&spid="+encodeURIComponent(sp_id)+"&cpid="+encodeURIComponent(cp_id)+"&first="+($scope.pagingOptions.currentPage-1)*$scope.pagingOptions.pageSize+"&max="+$scope.pagingOptions.pageSize;
	...
	$scope.selected_delete= function() {
		if ($scope.selectItem == null || $scope.selectItem.id === 1 ) {
			return true;
        } else if($scope.username=="sysadmin"){
            return false;
		}
        else if($scope.username==$scope.selectItem.name){
            return false;
        }
		else if($scope.selectItem.parentUserId != $scope.username){
			return true;
		}
		else {
			return false;
	...
	$scope.deleteItem = function() {
		...
		commonMethod.confirmDialog($scope.message).then(function(value) {
        	...
        	$http['post'](sharedProperties.getServerUrl()+"/rest/v1/security/user/remove",ids).success(function () {
...
.controller('userAddCtrl',['$scope', '$http', 'sharedProperties', '$routeParams', '$location', 'loginService', '$rootScope', '$q','commonMethod', function($scope, $http, sharedProperties,$routeParams,$location,loginService, $rootScope, $q,commonMethod){
	$scope.getRoles = function() {
		...
		$http.get(sharedProperties.getServerUrl()+'/rest/v1/security/role/list').  
	...
	$scope.saveItem = function() {
		...
		if ($scope.id == 0) {
			//$scope.data.id = 0;
			$http.post(sharedProperties.getServerUrl()+'/rest/v1/security/user/add',$scope.data)
			...
			//修改信息
			$http.post(sharedProperties.getServerUrl()+'/rest/v1/security/user/edit',$scope.data)
				.success(function(data) {
	...
	$scope.checkUserId = function() {
    	...
    	if ($scope.data.userId != null && $scope.data.userId != "" ) {
    		$http.get(sharedProperties.getServerUrl()+"/rest/v1/security/user/check/unique?name="+$scope.data.userId)
	...
	$scope.init = function() {
		...
		$q.all([$scope.getRoles()]).then(function(results){
			$scope.groups = results[0].list;
			//$scope.cps_tmp = angular.copy($scope.cps);
			$scope.resetItem();
		});
	};
	...
	$scope.editReset = function() {
		var url = sharedProperties.getServerUrl()+'/rest/v1/security/user/detail?id='+$scope.id;
...
.controller('userReviewCtrl', ['$scope','$http','sortKey','$location','sharedProperties','loginService','$routeParams','$q','$rootScope',function($scope,$http,sortKey,$location,sharedProperties,loginService,$routeParams,$q,$rootScope){
	...
	$scope.init = function() {
		$scope.url = sharedProperties.getServerUrl()+'/rest/v1/security/user/detail?id='+$scope.id;
...
.controller('userEditCtrl',['$scope', '$http', 'sharedProperties','$location', 'loginService', '$rootScope','commonMethod','$timeout', function($scope, $http, sharedProperties, $location, loginService, $rootScope,commonMethod,$timeout){
	...
	$scope.saveItem = function() {
		...
		$http.post(sharedProperties.getServerUrl()+"/rest/v1/security/user/password/edit",body)
			.success(function(data) {
