...
import cn.hoob.model.CategoryAssociation;
...
import cn.hoob.utils.TfidfUtil;
...
public class SimilaritySeriesModelApp {
	...
	public static  void main(String[] args) throws Exception {
		...
		//加载数据maping,补全栏目信息
		Dataset<Row> categoryAssociationRowDataset = sparkSession.read().
		format("jdbc").
		option("driver","com.mysql.jdbc.Driver").
		option("user",SysUtils.getSysparamString("epg.jdbc.user")).
		option("password",SysUtils.getSysparamString("epg.jdbc.password")).
		option("url",SysUtils.getSysparamString("epg.jdbc.url")).
		option("dbtable","category_association").load();
		Encoder<CategoryAssociation> categoryAssociationEncoder = Encoders.bean(CategoryAssociation.class);
		//合集过滤不满足条件的，转换成简易模型数据
		Dataset<CategoryAssociation> categoryAssociationDataset = categoryAssociationRowDataset.
				map(row-> CategoryAssociation.getCategoryAssociationModel(row),categoryAssociationEncoder).
		...
		Dataset<SeriesModel>seriesfullModelDataset=seriesfullDataset.map(
				row->SeriesModel.getSeriesModel(row),seriesModelEncoder).filter(row->row!=null);
		//构建StructType
		...
		//按时间排序值推荐前TopN(做分母)
		Dataset<Row>seriesMiniFullRowDataset=sparkSession.sql("select * from seriesfullmodel "
				+ "where status=1 and similartyText is not null order by updateTime desc limit "+limitNum);
		Dataset<Row> trainningTfidfDataset = TfidfUtil.tfidf(seriesMiniFullRowDataset.
				filter(row->(row.getAs("similartyText")!=null)),"similartyText","similartyTexts","\\|");
		Encoder<SimiartyDataFeatureModel> simiartyDataFeatureModelEncoder = Encoders.bean(SimiartyDataFeatureModel.class);
		Dataset<SimiartyDataFeatureModel> trainningFeaturedataset = trainningTfidfDataset.
				map(row->SimiartyDataFeatureModel.getSimiartyDataFeatureModel(row),simiartyDataFeatureModelEncoder);
		List<SimiartyDataFeatureModel> simiartyDataFeatureModelList=trainningFeaturedataset.javaRDD().collect();
		SimiartyDataFeatureModel[] simiartyDataFeatureModelArray = 
				new SimiartyDataFeatureModel[simiartyDataFeatureModelList.size()];
		simiartyDataFeatureModelList.toArray(simiartyDataFeatureModelArray);
		//把加载的与数据广播出去。。。
		final Broadcast<SimiartyDataFeatureModel[]> trainningData = sparkSession.sparkContext().
				broadcast(simiartyDataFeatureModelArray,
				ClassTag$.MODULE$.<SimiartyDataFeatureModel[]>apply(SimiartyDataFeatureModel[].class));
		Dataset<Row> seriesTfidDataset = TfidfUtil.tfidf(seriesfullRowDataset.
				filter(row->row.getAs("similartyText")!=null),"similartyText","similartyTexts","\\|");

		//求解合集之间的相似度
		Dataset<SimilartyDatas> similartyDataset= SysUtils.pickupTheTopSimilarShop(seriesTfidDataset, trainningData);

		JavaRDD<SimilartyData>similartyDataListRdd=similartyDataset.toJavaRDD().
				map(row->row.getSimilartyDatas()).flatMap(row->row.iterator());
		Dataset<SimilartyData> similartys=sparkSession.createDataset(similartyDataListRdd.rdd(),
