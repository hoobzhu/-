...
public class SimilaritySeriesOneByOneModelApp {
	...
	public static  void main(String[] args) throws Exception {
		...
		Dataset<Row> seriesRowDataset = sparkSession.read().
				jdbc(SysUtils.getSysparamString("epg.jdbc.url"),"series", properties);
		...
		Dataset<Row> categoryAssociationRowDataset = sparkSession.read().
				jdbc(SysUtils.getSysparamString("epg.jdbc.url"),"category_association", properties);
		...
		Dataset<SeriesModel>seriesfullModelDataset=seriesfullDataset.
				map(row->SeriesModel.getSeriesModel(row),seriesModelEncoder).filter(row->row!=null);
		...
		//按时间排序值推荐前TopN(做分母)
		Dataset<Row> seriesMiniFullTfidfDataset = TfidfUtil.tfidf(seriesMiniFullRowDataset.
				filter(row->row.getAs("similartyText")!=null),"similartyText","similartyTexts","\\|");
		//分子
		Dataset<Row> trainningTfidfDataset = TfidfUtil.tfidf(seriesfullRowDataset.
				filter(row->row.getAs("similartyText")!=null),"similartyText","similartyTexts","\\|");
		...
		while(step<count){
			...
			Dataset<Row> partTrainningTfidfDataset=sparkSession.sql("select * from "
					+ "(select *,row_number() over (partition by contentId order by id desc) "
					+ "as rank from trainningTfidfDataset ) "
			...
			/*	ThreadPoolUtil.execute(new  Runnable() {
					...
					public void run() {*/
						...
						Dataset<Row> similartysFiter=  sparkSession.sql("" +
								...
								+ "select * from ("
								+ "select contentId,similarContentId,similarty, row_number() over(partition by "
								+ "contentId order by similarty desc) as rn "
