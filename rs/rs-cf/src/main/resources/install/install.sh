...
TOMCAT_HOME="/opt/fonsview/3RD/tomcat7.0.63"
tomcatDir="tomcat"
if [ ! -d "/opt/fonsview/3RD/tomcat" ]; then
        tomcatDir="tomcat7.0.63"
fi

TOMCAT_HOME="/opt/fonsview/3RD/"$tomcatDir

BASE_PATH="/opt/fonsview/NE/rs/rs-cf"
...
rm -rf $TOMCAT_HOME/webapps/rscf.war
rm -rf $TOMCAT_HOME/webapps/rscf
cp -rf ../rs-cf*.war $TOMCAT_HOME/webapps/rscf.war
