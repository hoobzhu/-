...
import cn.hoob.rs.model.Rating;
import cn.hoob.rs.model.SeriesModel;
import cn.hoob.rs.model.UserModel;
...
public class ALSCrossValidationModelApp {
	...
	public static void main(String[] args) throws Exception {
		...
		Dataset<SeriesModel> seriesModelDataset = seriesRowDataset.map(row-> 
			  SeriesModel.getSeriesModelByIdAndContentId(row),seriesModelEncoder)
		...
		Dataset<Rating>ratingSimpleDataset=logDataset.map(row->Rating.parseSimpleRating(row), ratingEncoder).filter(rating -> rating != null);
		...
		properties.put("user",SysUtils.getSysparamString("aaa.jdbc.user"));
		properties.put("password",SysUtils.getSysparamString("aaa.jdbc.password"));
		Dataset<Row> reuserRowDataset = sparkSession.read().jdbc(SysUtils.getSysparamString("aaa.jdbc.url"),"userprofile", properties);
		...
		Dataset<Row>ratingfullDataset=sparkSession.sql("select user.id as uId,s.id as sId,rslog.rating as rating from ratingsimple rslog "
        ...
        Dataset<Row> userreconmenddata=model.recommendForAllUsers(SysUtils.getSysparamInteger("recommendForAllUsers","20"));
        //推荐数据视图,输出到RS DB
