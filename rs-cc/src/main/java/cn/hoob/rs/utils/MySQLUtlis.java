...
public class MySQLUtlis {
	...
	public static void excuteBatchReUser(Dataset<Row> data){ 
		data.foreachPartition(partition->{
			...
			PreparedStatement pstmt=connection.prepareStatement("INSERT INTO `user_recommendation` (`contentIds`, `userId`) VALUES (?,?) ON DUPLICATE KEY UPDATE `contentIds` = values(`contentIds`)");
	...
	public static void excuteBatchReSimilarity(Dataset<Row> data){ 
		data.foreachPartition(partition->{
			Connection connection = getConnection();
			PreparedStatement pstmt=connection.prepareStatement("INSERT INTO `similar_recommendation` (`contentIds`, `contentId`) VALUES (?,?) ON DUPLICATE KEY UPDATE `contentIds` = values(`contentIds`)");
			connection.setAutoCommit(false);
			int i=1;
			while(partition.hasNext()){
				Row row=partition.next();
	...
	public static void excuteBatchReTopN(Dataset<Row> data){ 
		data.foreachPartition(partition->{
			Connection connection = getConnection();
			PreparedStatement pstmt=connection.prepareStatement("INSERT INTO `topn_recommendation` (`playCount`, `programType`, `contentId`) VALUES (?,?,?) ON DUPLICATE KEY UPDATE `playCount` = values(`playCount`)");
			connection.setAutoCommit(false);
			int i=1;
			while(partition.hasNext()){
				Row row=partition.next();
