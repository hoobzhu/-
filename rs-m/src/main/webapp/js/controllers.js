...
.controller('indexCtrl',['$log','loginService','$scope','$location', '$translate','$rootScope','sharedProperties','$http','$window','$q','$interval','$timeout','commonMethod','rootService',function($log,loginService, $scope, $location, $translate,$rootScope,sharedProperties,$http,$window,$q,$interval,$timeout,commonMethod,rootService) {
	    ...
	    //获取权限列表
	    $rootScope.rootPris = rootService;
	    console.log($rootScope.rootPris);
...
/**
 * 获取distributeConfig对象，保存分发配置的名称（拼接）和可用数量
 */
function getConfig(distributeConfig){
	 var config = {"cdn":{"name":"","size":0},"bms":{"name":"","size":0},"cms":{"name":"","size":0},
	 			"tvgwVod":{"name":"","size":0},"drm":{"name":"","size":0},"tcgs":{"name":"","size":0}}
		//计算可用(size,可用的分发配置名称)
		if(distributeConfig){
			//cdn
			var cdnList = distributeConfig.cdnList;
			if(cdnList){
				cdnList.sort(function(a,b){
					return a.position-b.position
				});
				distributeConfig.cdnList.forEach(function(cdn){
					config.cdn.size += 1;
					config.cdn.name += cdn.name+"/"
				});
				if(config.cdn.name!=""){
					config.cdn.name = config.cdn.name.substring(0,config.cdn.name.length-1);
				}
			}
			//bms
			var bmsList = distributeConfig.bmsList;
			if(bmsList){
				bmsList.sort(function(a,b){
					return a.position-b.position
				});
				distributeConfig.bmsList.forEach(function(bms){
					config.bms.size += 1;
					config.bms.name += bms.name+"/"
				});
				if(config.bms.name!=""){
					config.bms.name = config.bms.name.substring(0,config.bms.name.length-1);
				}
			}
			//cms
			var cmsList = distributeConfig.cmsList;
			if(cmsList){
				cmsList.sort(function(a,b){
					return a.position-b.position
				});
				distributeConfig.cmsList.forEach(function(cms){
					config.cms.size += 1;
					config.cms.name += cms.name+"/"
				});
				if(config.cms.name!=""){
					config.cms.name = config.cms.name.substring(0,config.cms.name.length-1);
				}
			}
			//tvgwVod
			var tvgwVodConfig = distributeConfig.tvgwVodConfig;
			if(tvgwVodConfig){
				config.tvgwVod.size = 1;
				config.tvgwVod.name = tvgwVodConfig.name;
			}
			//tcgs
			var tcgsList = distributeConfig.transCodeList;
			if(tcgsList){
				tcgsList.sort(function(a,b){
					return a.position-b.position
				});
				distributeConfig.tcgsList.forEach(function(tcgs){
					config.tcgs.size += 1;
					config.tcgs.name += tcgs.name+"/"
				});
				if(config.tcgs.name!=""){
					config.tcgs.name = config.tcgs.name.substring(0,config.tcgs.name.length-1);
				}
			}
			//drm
			var drmConfig = distributeConfig.drmConfig;
			if(drmConfig){
				config.drm.size = 1;
				config.drm.name = drmConfig.name;
			}
...
function getMediaType(media){
	if(media=='SERIES'){
		return 2;
	}else if(media=='PROGRAM'){
		return 1;
	}else if(media=='MOVIE'){
		return 10;
	}else if(media=='CHANNEL'){
		return 3;
	}else if(media=='SCHEDULE'){
		return 13;
	}else if(media=='SERVICE'){
		return 24;
	}else if(media=='CATEGORY'){
		return 23;
	}else if(media=='SERVICEA'){
		return 202;
	}else if(media=='CATEGORYA'){
		return 203;
	}else if(media=='WEB_EPGVERION'){
		return 29;
	}
}
