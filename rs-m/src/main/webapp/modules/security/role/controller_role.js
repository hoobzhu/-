...
.controller('roleListCtrl', ['$rootScope','$scope', '$http', 'sharedProperties','loginService','$window','$routeParams','commonMethod',function($rootScope,$scope, $http, sharedProperties, loginService,$window,$routeParams,commonMethod) {
	$scope.getPageData = function() {
		...
		var url = sharedProperties.getServerUrl()+"/rest/v1/security/role/list?id="+encodeURIComponent($scope.queryOptions.id)+"&name="+encodeURIComponent(name)+"&first="+($scope.pagingOptions.currentPage-1)*$scope.pagingOptions.pageSize+"&max="+$scope.pagingOptions.pageSize;
	...
	$scope.deleteItem = function() {
		...
		commonMethod.confirmDialog($scope.message).then(function(value) {
        	...
        	$http["post"](sharedProperties.getServerUrl()+"/rest/v1/security/role/remove",ids).success(function (data) {
...
//		}
		...
		if ($scope.id == 0) {
			$http.post(sharedProperties.getServerUrl()+'/rest/v1/security/role/add',$scope.data)
			...
			$http.post(sharedProperties.getServerUrl()+'/rest/v1/security/role/edit',$scope.data)
				.success(function(data) {
	...
	$scope.init = function() {
		...
		//获取权限列表
		$http.get(sharedProperties.getServerUrl()+'/rest/v1/security/privilege/list').success(function(data){
			var list = data.list;
	...
	$scope.editReset = function() {
		$http.get(sharedProperties.getServerUrl()+'/rest/v1/security/role/detail?id='+$scope.id).success(function(data){
			$scope.data = data.vo;
