	...
	public PrivilegeVO getPrivilegeVo(Serializable entityId) {
		if (entityId != null) {
			...
			if (privilege != null) {
				...
				PrivilegeVO parent_priv = new PrivilegeVO();
				if (privilege.getId() == 1) {
					privilegeVO.setParent(null);
				} else {
					BeanUtils.copyProperties(privilege.getParent(), parent_priv);
					privilegeVO.setParentId(parent_priv.getId());
					privilegeVO.setParent(parent_priv);
				}
				return privilegeVO;
			}
		}
