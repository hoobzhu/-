...
public class UserDaoImpl extends BaseDaoImpl implements UserDao{
	...
	public UserVO findUserVOByUserId(String userId) {
		...
		params.put("userId", userId);
		try{
			UserVO userVO = this.get("select new com.hoob.rs.security.vo.UserVO(u.id,u.userId,u.cpId,u.spId ) from User u where u.userId = :userId", params);
			return userVO;
		}catch(Exception ex){
			logger.error(ex);
			return null;
...
//		StringBuffer jpql = new StringBuffer("select new com.hoob.cms.security.vo.UserVO(u.id,u.roles.id )");
//		StringBuffer jpql = new StringBuffer("select new com.hoob.cms.security.vo.UserVO(u.id,u.name,u.role,u.nickName,u.lastLoginTime,u.loginTimes,u.enable, ");
...
//		StringBuffer stringBuffer = new StringBuffer("select new com.hoob.cms.security.vo.UserVO(u.roles) from User u,Role r where 1=1 and r.id in elements(u.roles) ");
//		StringBuffer stringBuffer = new StringBuffer("select new com.hoob.cms.security.vo.UserVO(u.roles) from User u left join u.roles r where 1=1 ");
