...
import com.hoob.rs.sys.vo.ConfigValueMappingVO;
...
public class SysConfigServiceImpl extends BaseServiceImpl implements SysConfigService {
	...
	public boolean isBroadcastControl() {
		...
		Boolean IBCP=false;
		try {
			SysConfig sysc = find(ConfigKey.IBCP.toString());
			if(sysc!=null){
				@SuppressWarnings("unchecked")
				Map<String,String> map = JsonUtils.json2Obj(sysc.getValue(), Map.class);
				if(map != null){
					IBCP = Boolean.parseBoolean(map.get("isBroadcastControl"));
				}
			}
		} catch (Exception e) {
			logger.error(e.getMessage(),e);
	...
	public boolean getGlobalDefaultService() {
		Boolean GLOBAL_DEFAULT_SERVICE=false;
		try {
			SysConfig sysc = find(ConfigKey.GLOBAL_DEFAULT_SERVICE.toString());
			if(sysc!=null){
				@SuppressWarnings("unchecked")
				Map<String,String> map = JsonUtils.json2Obj(sysc.getValue(), Map.class);
				if(map != null){
					GLOBAL_DEFAULT_SERVICE = Boolean.parseBoolean(map.get("GLOBAL_DEFAULT_SERVICE"));
				}
			}
		} catch (Exception e) {
			logger.error(e.getMessage(),e);
		}
		return GLOBAL_DEFAULT_SERVICE;
	}
	...
	/**
	 * @Title getAuditSwitch
	 * @Description   获取审核开关配置
	 * @param 
	 * @return SysConfigService
	 * @throws 
	 */
	...
	public boolean getAuditSwitch() {
		Boolean APPROVED_CONFIG=false;
		try {
			SysConfig sysc = find(ConfigKey.APPROVED_CONFIG.toString());
			if(sysc!=null){
				@SuppressWarnings("unchecked")
				Map<String,String> map = JsonUtils.json2Obj(sysc.getValue(), Map.class);
				if(map != null){
					APPROVED_CONFIG = Boolean.parseBoolean(map.get("useCheckWorkflow"));
				}
			}
		} catch (Exception e) {
			logger.error(e.getMessage(),e);
	 ...
	 */
	@Override
	public String matchValue(String configKey, String temp) {
		if(StringUtils.isEmpty(configKey)||StringUtils.isEmpty(temp)){
			return null;
		}
		//获取对应的系统配置
		try {
			...
			@SuppressWarnings("unchecked")
			List<ConfigValueMappingVO> tvo=  JsonUtils.json2ObjList(sysc.getValue(), ConfigValueMappingVO.class);
			if(tvo==null){
				return null;
			}
			for(ConfigValueMappingVO vo:tvo){
				if(vo.getMappingValues().contains(temp)){
					return vo.getValue();
				}
			}
			return null;
